...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Mobile handled via JS - see .mobile-* classes */
  .mobile-shell { display: block !important; position: relative; overflow: hidden; height: 100vh; }
  .mobile-left { width: 100% !important; min-width: 0 !important; height: 100vh; position: absolute !important; top: 0; left: 0; right: 0; bottom: 0; transition: transform 0.25s ease; }
  .mobile-left-hidden { transform: translateX(-100%) !important; pointer-events: none !important; }
  .mobile-right { display: flex !important; flex-direction: column !important; position: absolute !important; top: 0; left: 0; right: 0; bottom: 0; background: var(--bg) !important; z-index: 50; transition: transform 0.25s ease; transform: translateX(100%); flex: none !important; width: 100% !important; }
  .mobile-right-open { transform: translateX(0) !important; }
  ...
  .show-back { display: inline-flex !important; align-items: center; gap: 5px; }
...
export default function App() {
  ...
  // Detect mobile: use logical pixels (innerWidth / dpr), or Capacitor native flag
  const getIsMobile = () => {
    const logical = window.innerWidth / (window.devicePixelRatio || 1);
    const screenLogical = window.screen.width / (window.devicePixelRatio || 1);
    const isCap = !!(window.Capacitor);
    return isCap || logical < 600 || screenLogical < 600;
  };
  ...
  const closeRight = () => { setRightMode("welcome"); };
  ...
  return (
    <>
      <style>{STYLES}</style>
      <div className={isMobile ? "shell mobile-shell" : "shell"}>
        {/* ── LEFT ── */}
        <div className={isMobile ? `left mobile-left ${isRightOpen ? "mobile-left-hidden" : ""}` : "left"}>
          <div className="hdr">
            <div className="hdr-top">
              ...
              <div style={{textAlign:"right"}}>
                ...
                <div style={{fontFamily:"Barlow Condensed",fontSize:13,color:"var(--accent)",letterSpacing:2,textAlign:"right",marginTop:4,fontWeight:700,textTransform:"uppercase"}}>v{APP_VERSION}</div>
              </div>
            </div>
            {updateAvailable && (
        ...
        {/* ── RIGHT ── */}
        <div className={isMobile ? `right mobile-right ${isRightOpen ? "mobile-right-open" : ""}` : `right`}>
          {rightMode==="welcome" && (
          ...
          {rightMode==="videos" && (
            <>
              <div className="rhdr">
                {isMobile && <button className="back-btn show-back" onClick={()=>setRightMode("welcome")}>← Back</button>}
          ...
          {rightMode==="mobility" && activeMobVideo && (
            <>
              <div className="rhdr">
                {isMobile && <button className="back-btn show-back" onClick={()=>{setRightMode("welcome");setActiveMobVideo(null);}}>← Back</button>}
                <div className="rhdr-row">
                  <div>
